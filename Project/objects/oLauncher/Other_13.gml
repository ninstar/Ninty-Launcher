...
text_settings_language[7] = "Русский";
...
if(language == 0){
    ...
    switch(os_get_language()){
        ...
        case("ru"): lang = 7; break; //RUSSIAN (Русский)
        default: lang = 1;
    }
}
else
...
text_dialog[0] = "Введите название игры:";
text_ctrl[0] = "Запустить";
text_ctrl[1] = "Закрыть";
text_ctrl[2] = "Переместить";
...
text_addgame[0] = "[ Использовать как ярлык? ]: Эта настройка полезна для приложений, которые не являются играми, или программ, которые используют потентованные лаунчеры (Например инструменты) | УЧТИТЕ: Если вы выберите эту настройку, то время игры не будет записано, а лаунчер не закроется.";
text_addgame[1] = "[ Использовать как перенаправление? ]: Эта настройка полезна для эмуляторов, вы можете запустить один или более ROMов прямо из Ninty. | УЧТИТЕ: В следующем шаге вам потребуется выбрать папку в которой расположены ROMы.";
...
text_settings[5] = "Проверять наличие обновлений";
...
text_settings_language[0] = "Системный";  
text_settings_uirender[0] = "Совместимый";
text_settings_uirender[1] = "Современный";
text_settings_windowmode[0] = "Такое же, как и Ninty";
text_settings_windowmode[1] = "Обычное";
text_settings_windowmode[2] = "Полноэкранное";
text_settings_gamepriority[0] = "Обычный (Рекоммендуется)";
text_settings_gamepriority[1] = "Высокий";
text_settings_gamepriority[2] = "Низкий";
...
guide[1] = @"Ninty - это лаунчер для запуска ваших игр,
основанный на интерфейсе Nintendo Switch .";

guide[2] = @"Кроме того, Ninty поддерживает программы,
которые не являются играми.";

guide[3] = "Ну, давайте же начнём!";

guide[4] = @"Это кнопоки функций, с их помощью вы можете добавить, поменять
или убрать игру, а также изменить стиль лаунчера.";

guide[5] = @"Поосторожнее с кнопкой в виде мусорного ведра, она убирает
приложения из библиотеки! Вы же не хотите потерять своё время игры...";

guide[6] = @"Но не волнуйтесь! Чтобы избежать таких ситуаций, 
у нас есть функция скрытия интерфейса.";

guide[7] = @"Нажмите 3 раза на эту кнопку, чтобы спрятать основные функции.
Так можно избежать случайного передвижения игр и приложений.";

guide[8] = @"Чтобы вернуть их на свои места,
нажмите на то же место ещё 3 раза.";

guide[9] = @"Здесь можно найти всю информацию о Ninty,
а также версию лаунчепа и тех, кто помог разработке.";

guide[10] = "Здесь располагаются меню настроек лаунчера.";

guide[11] = @"Вот и меню. Данный пункт откроет
папку с вашим профилем Ninty.";

guide[12] = @"У каждого пользователя компьютера есть свой собственный профиль,
а также своя папка с этим профилем. Это значит, что у каждого
будет свой список игр, стиль и настройки.";

guide[13] = @"Если вы планируете поменять аккаунт или компьютер
...
скачивать последнюю базу данных об играх при каждом запуске.";

guide[15] = @"Каждый раз, когда лаунчер скачивает что-либо
из базы данных, будет играть анимация.";

guide[16] = @"Если вы включите эту настройку, я буду
уведомлять вас о новых обновлениях Ninty!";

guide[17] = "Эта настройка устонавливает приоритет обработки игры.";

guide[18] = @"Я бы не советовала менять эту настройку, так как,
в зависимости от запущенной игры, она может сильно повлиять на
производительность вашего компьютера. Я серьёзно!";

guide[19] = @"Эта настройка позволяет вам изменить размер окна игры
после её запуска, однако это работает не во всех играх.";

guide[20] = @"Некоторые могут заставить играть в оконном режиме, что
не всегда хорошо, в зависимости от вашего предпочтения...";

guide[21] = @"С помощью этой настройки вы можете отключить все 
звуки лаунчера, кроме музыки на заднем фоне.";

guide[22] = @"Эта настройка позволяет вам настроить метод отображения
интерфейса. Это очень важно, так как на некоторых компьютерах
могут происходить графические баги.";

guide[23] = @"'Совместимый' режим решает большинство
подобных проблем, и это стардартная настройка";

guide[24] = @"Но я рекомендую использовать 'современный' режим.
В нём лучше визуальные эффекты и производительность.";

guide[25] = "И наконец, эта настройка меняет язык лаунчера.";

guide[26] = "Не беспокойтесь, я знаю много языков!";

guide[27] = @"Если вам ещё раз понадобится помощь гида, выберите
вот эту функцию, и буду рада помочь!";
...
на вашем профиле во время игры!";

guide[30] = "Тут есть различные темы, наслаждайтесь!";
 
guide[31] = "Доступно новое обновление:  "+vnew+@"
Ваша текущая версия: "+vold;
